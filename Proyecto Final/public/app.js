...
let photoshop = document.getElementById("c++");
crearBarra(photoshop);
let php = document.getElementById("pascal");
crearBarra(php);
...
function efectoHabilidades(){
    ...
    if(distancia_skills>=300 && entro==false){
        ...
        const intervalCss = setInterval(function(){
            pintarBarra(wordpress, 11, 2, intervalCss);
        },100);
        const intervalC = setInterval(function(){
            pintarBarra(photoshop, 10, 3, intervalC);
        },100);
        const intervalPascal = setInterval(function(){
            pintarBarra(php, 5, 4, intervalPascal);
        },100);
        const intervalOffice = setInterval(function(){
            pintarBarra(ilustrator, 15, 5, intervalOffice);
        },100);
    }
}
